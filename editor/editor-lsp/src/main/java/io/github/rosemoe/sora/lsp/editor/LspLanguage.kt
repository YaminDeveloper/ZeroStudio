...
class LspLanguage(var editor: LspEditor) : Language {
  ...
  init {
    ...
    completionItemProvider = CompletionItemProvider { completionItem, eventManager, prefixLength ->
  ...
  override fun requireAutoComplete(
    ...
    val documentChangeEvent = editor.eventManager.getEventListener<DocumentChangeEvent>() ?: return
    ...
    val serverResultCompletionItems =
        editor.coroutineScope.future {
          editor.eventManager
              .emitAsync(EventType.completion, position)
  ...
  private fun computePrefix(text: ContentReference, position: CharPosition): String {
    val triggers =
        editor.completionTriggers.filterNot { trigger ->
    ...
    val delimiters = triggers.toMutableList().apply { addAll(listOf(" ", "\t", "\n", "\r")) }
